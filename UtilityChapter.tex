...
\subsubsection{Cost indicators}\mbox{}\\[-5ex]
\begin{itemize}
...
\subsubsection{Benefit indicators}\mbox{}\\[-5ex]
\begin{itemize}
...
\subsubsection{Other important utility factors for sub-balances}\mbox{}\\
...
\subsubsection{Traffic model simulation data}\label{subsub:simData}\mbox{}\\
...
{%
	...
	$\Delta$MPT person dist. & \Delta D_{\text{pers,MPT}} \text{ [km/a]} & (D_{\text{pers,MPT|metroScen}} - D_{\text{pers,MPT|baseScen}}) \cdot 250 \\
	$\Delta$PT person dist. & \Delta D_{\text{pers,PT}} \text{ [km/a]} & (D_{\text{pers,PT|metroScen}} - D_{\text{pers,PT|baseScen}}) \cdot 250\\
	$\Delta$Other person dist. & \Delta D_{\text{pers,Oth}} \text{ [km/a]} & (D_{\text{pers,Other|metroScen}} - D_{\text{pers,Other|baseScen}}) \cdot 250\\
	$\Delta$Total PT trav. time & \Delta t_{pers,PT} \text{ [h/a]} & (t_{\text{pers,PT|metroScen}} - t_{\text{pers,PT|baseScen}}) \cdot 250\\
	$\Delta$Total MPT trav. time & \Delta t_{pers,MPT} \text{ [h/a]} & (t_{\text{pers,MPT|metroScen}} - t_{\text{pers,MPT|baseScen}}) \cdot 250\\
	$\Delta$Total Other trav. time & \Delta t_{pers,Oth} \text{ [h/a]} & (t_{\text{pers,Other|metroScen}} - t_{\text{pers,Other|baseScen}}) \cdot 250\\
...
%		\hline \hline
		...
		C_{statUG} & Construction cost of a new UG metro station (double-platform) = 160 Mio CHF\\ 
		  & \multicolumn{1}{p{13.0cm}}{\raggedright Note that the costs do not include the construction of the actual track (considered separately by the link construction cost), but only the station infrastructure itself. The value is derived from averaging over construction costs of comparable stations in Paris \citet{DRIEA12} and corrected for the PPP ratio between France and Switzerland of 1.5 from \citet{OECD17}. The resulting value refers to the cost of approximately one kilometer of new UG links, which is a reasonable estimate for a UG station as it may be described as two parallel 500 meter long tunnels.}\\ 
		C_{statUGExt} & \multicolumn{1}{p{13.0cm}}{\raggedright Cost to extend an existing UG metro station by platforms and other infrastructure to match increased capacity demands. This value is generalized as $3/4$ of the construction cost of a new UG station above to be 120 Mio CHF.}\\ 
		C_{statOG} & Construction cost of a new OG metro station (double-platform) = 55 Mio CHF\\
		& \multicolumn{1}{p{13.0cm}}{\raggedright Note that the costs do not include the construction of the actual track, which is listed separately, but only the station infrastructure itself. The costs are derived from comparable (double-platform) projects, see \citet{KONOL18}, \citet{SIG17}}\\ 
		C_{statOGExt} & \multicolumn{1}{p{13.0cm}}{\raggedright Cost to extend an existing OG metro station by platforms and related infrastructure to match increased capacity demands. This value is generalized as $3/4$ of the construction cost of a new OG station above to be 41 Mio CHF.}\\ 

		C_{conUG} & Construction cost of new UG links = 150 Mio CHF/Km\\
				  & \multicolumn{1}{p{13.0cm}}{\raggedright This value results directly from comparable underground train link construction projects such as those featured in \citet{INFRASUISSE2017}}\\  % or the lately completed Weinberg-Tunnel as a part of Zurich's new "Durchmesser-Linie", which is estimated to have costed 150 Mio CHF/Km.
		C_{conUGdev} & Extension of existing UG train link (e.g. adjacent tunnel) = 112 Mio CHF/km; \\
		  & \multicolumn{1}{p{13.0cm}}{\raggedright This value is generalized as $3/4$ of the construction cost of a new UG link above and would come in place for instance for the existing tunnel between Zurich HB - Stadelhofen, whose capacity would have to be extended by means of new parallel tracks and therefore an appropriate expansion of the tunnel. Extending a link is cheaper due to easier access of the construction site and possibilities to use some of the existing infrastructure, e.g. diversion tunnels or power supplies.}\\ 
		...
		& \multicolumn{1}{p{13.6cm}}{\raggedright
		 ...
		 The operational costs are summed for each year and discounted by eq. \ref{eq:discount}.} \\ \hline
		%%
		\rowcolor{lightgray!60}\mathbf{C_{main/rep}} & Maintenance and repair cost\\
		  & $=1/3\cdot C_{ops}$ generalized from CoMET data in \cite{LU16}\\ \hline
\end{longtable}
...
\begin{longtable}{L | l}
	...
	\endlastfoot
		...
		\rowcolor{lightgray!60}\mathbf{B_{time}} & \multicolumn{1}{p{14.2cm}}{\raggedright Travel time gains $ = x \cdot (u_{\text{tPT}}\cdot\Delta t_{pers,PT} + u_{\text{tMPT}}\cdot\Delta t_{pers,MPT} + u_{\text{tOther}}\cdot\Delta t_{pers,Other})$}\\
		u_{\text{timePT}} & Utility of a person's PT travel time savings = 14.43 CHF/h (\cite{VSS_Norm_KNA641821_2006})\\
		u_{\text{timeMPT}} & Utility of a person's MPT travel time savings = 23.29 CHF/h (\cite{VSS_Norm_KNA641821_2006})\\ 
		 & \multicolumn{1}{p{14.2cm}}{\raggedright While the utility of time savings for PT is assumed to be stable, the utility of time savings in the MPT mode may be subjected to a decline as with driver-less technology the "driver" can pursue other value bringing activities while traveling as is with PT. However, as this thesis focuses on improving PT network design and therefore car travel time is affected only indirectly, the utility parameters are assumed constant.} \\
		 u_{\text{timeOther}} & Utility of a person's walk/bike time savings = 33.20 CHF/h (\cite{Becker2017UtilWalkX})\\ \hline \hline
		%%	
		...
		e_{\text{p,MPT}} & External cost for one MPT person kilometer = 0.077 CHF/personKm (\cite{ARE2015}) \\ \hline
...
add all evolutionary operators here
keep this section generic, i.e. not limited to Metro 
