...
public class NetworkCreator {
	...
	// TODO METRO Network
		...
		OutputTestingImpl.createNetworkFromCustomLinks(mostFeasibleLinks, network, "zurich_1pm/created_input/zurich_network_strongestLinks.xml", "zurich_1pm/created_input/zurich_network_FacilityCoordNodes.xml"); // links are merged in one new network and facilities are placed as nodes in another new network		
		Coord zurich_NetworkCenterCoord = new Coord(2683099.3305, 1247442.9076);
		double minRadiusFromCenter = 1563.7356*0.67;
		double maxRadiusFromCenter = 1563.7356*2.00;
		Map<Id<Link>,CustomLinkAttributes> feasibleTerminalCandidateLinks= DominantStopFacilitiesOnLinks.findFeasibleTerminalLinks(network, new PT_StopTrafficCounter(), iterationToRead, mostFeasibleLinks, zurich_NetworkCenterCoord, minRadiusFromCenter, maxRadiusFromCenter);	// find most frequent links from all network links
		OutputTestingImpl.createNetworkFromCustomLinks(feasibleTerminalCandidateLinks, network, "zurich_1pm/created_input/zurich_network_TerminalCandidateLinks.xml", "zurich_1pm/created_input/zurich_network_TerminalCandidateFacilitiesNodes.xml"); // links are merged in one new network and facilities are placed as nodes in another new network
	}
