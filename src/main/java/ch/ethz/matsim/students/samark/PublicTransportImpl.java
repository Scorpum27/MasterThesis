...
public class PublicTransportImpl {
	...
	public static List<TransitRouteStop> createAndAddNetworkRouteStops(TransitSchedule transitSchedule, Network network, NetworkRoute networkRoute, String defaultPtMode, double stopTime, double maxVehicleSpeed, boolean blocksLane){
		...
		for (Id<Link> linkID : NetworkCreatorImpl.networkRouteToLinkIdList(networkRoute)) {
	...
	public static Network mergeRoutesNetworkToOriginalNetwork(Network routesNetwork, Network originalNetwork, String fileName) {
		Network mergedNetwork = ScenarioUtils.createScenario(ConfigUtils.createConfig()).getNetwork();
		
		NetworkCreatorImpl.copyNetworkToNetwork(routesNetwork, mergedNetwork);
		NetworkCreatorImpl.copyNetworkToNetwork(originalNetwork, mergedNetwork);
		
		for (Node node : routesNetwork.getNodes().values()) {
			Id<Link> originalRefLinkId = NetworkCreatorImpl.orginalLinkFromMetroNode(node.getId());
